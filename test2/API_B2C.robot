...
Post_login 
    ${header}=  Create dictionary  Content-Type=application/json  accept=*/*
    ${body}=  create dictionary   email_or_phone=${email}  password=pmbfsaaa20   os=android  device_token=xxxx-xxxx-xxxx-xxxx   version=1.0.0
...
#Generate Tunisian Car Plate Number
    ...
    ${body}=  create dictionary   name=Car_test  color=black   plate_code=${plate_code}   plate_number=${plate_number}  type_id=4   brand_id=8  carParkIds=[${3}]
...
Post_pre_booking
   ${json_resp}=  Set Variable  ${resp.json()}
    ...
    ${body}=  create dictionary   parking_id=1   vehicle_id=1      start_time=2024-01-10 13:00   end_time=2024-01-10 16:00  price=500
